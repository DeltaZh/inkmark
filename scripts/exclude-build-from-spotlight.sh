#!/usr/bin/env bash
# 避免 Spotlight / 启动台搜到构建目录里的 Inkmark.app（正式应用在 /Applications）
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
TARGET="$ROOT/src-tauri/target"
APP="$TARGET/release/bundle/macos/Inkmark.app"
...
echo "Spotlight 当前 app.inkmark 路径:"
mdfind "kMDItemCFBundleIdentifier == 'app.inkmark'" 2>/dev/null || true
